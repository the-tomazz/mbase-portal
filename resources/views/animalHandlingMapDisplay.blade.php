...
<script>
	var markers = [
		...
		{{'}'}},
		title: '<b>{{__("Name")}}:</b> {{$animalHandling->animal_name}}<br><b>{{__("Handling date")}}:</b> {{(new DateTime($animalHandling->animal_handling_date))->format("m.d.Y H:i")}}<br><b>{{__("Type of conflict animal removal")}}:</b> {{$animalHandling->conflict_animal_removal_list == null ? "" : $animalHandling->conflict_animal_removal_list->name}}',
		{{'}'}},
		@endforeach
	];
